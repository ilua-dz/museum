...
document.getElementById('slide-number').textContent =
  '0' + welcomeSlider.activeIndex;

welcomeSlider.on('slideChange', function () {
  document.getElementById('slide-number').textContent = `0${
    welcomeSlider.realIndex + 1
  }`;
});
...
function compareInit() {
  ...
  if (window.innerWidth < 769)
    slider.style.top = h * 0.52 - slider.offsetHeight / 2 + 'px';
}
let pressed = 0;
...
function slide(pos) {
  ...
  slider.style.left =
    compareOverlay.offsetWidth - slider.offsetWidth / 2 + 'px';
}
...
videoPlayer.addEventListener('timeupdate', () => {
  if (videoIsPlaying) {
    videoTimeProgress.value = Math.floor(
...
videoTimeProgress.addEventListener('change', () => {
  videoPlayer.currentTime = Math.floor(
    (videoTimeProgress.value * videoPlayer.duration) / 100
  );
  changeProgress(videoTimeProgress);
});
  ...
  if (preventAction) {
    switch (e.code) {
      ...
      case 'Period':
        ...
        break;
      case 'Comma':
        if (videoSpeed > 0.25) videoSpeed -= 0.25;
        videoSpeedChange();
        break;
      case 'Escape':
...
const videoSliderPagination = document.querySelector(
  '.video-slider-pagination-container'
);

const videoSliderSizing = () => {
  videoSliderSlide.forEach(
    (i) => (i.style.height = i.offsetWidth * 0.561947 + 'px')
  );
};
...
for (let i = 1; i < 16; i++) {
  if (i === 5 || i === 11)
    randomImages.splice(i, 0, `<div class="spacer"></div>`);
}
...
const galleryAnimation = () => {
  galleryImages.forEach((i) => {
    ...
    const imgOffset =
      i.getBoundingClientRect().top + scrollY ||
...
const exhibitionTypeRadioButtons = document.querySelectorAll(
  '.tickets-type-choise-input'
);
...
for (let i = 0; i < availableTime.length; i++)
  availableTime[
    i
  ] = `<option value="${availableTime[i]}">${availableTime[i]}</option>`;
...
const setAmountAndPrice = () => {
  ...
  switch (exhibitionType) {
    case '1':
      exhibitionString.textContent = exhibTypes[0];
      break;
    case '1.25':
      exhibitionString.textContent = exhibTypes[1];
      break;
    case '2':
      exhibitionString.textContent = exhibTypes[2];
      break;
  }

  exhibitionTypeSelect.value = exhibitionType;
  exhibitionTypeRadioButtons.forEach((i) => {
    if (i.value === exhibitionType) i.checked = true;
  });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  ticketTimeInput.textContent =
    localStorage.getItem('ticketTime') || '-- -- --';
  ticketDateString.textContent =
...
mapboxgl.accessToken =
  'pk.eyJ1IjoiaWx1YS1keiIsImEiOiJja3VtZ2U3NWIwZTY2Mm9waXp3anBwNGpuIn0.bUEBVKqZ5IAe5T1_vQSZAA';
